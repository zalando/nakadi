...
public class PartitionsControllerTest {
    ...
    public void before() {
        ...
        final PartitionsController controller = new PartitionsController(topicRepositoryMock, eventTypeRepository);
    ...
    public void whenListPartitionsForWrongTopicThenNotFound() throws Exception {
        ...
        when(eventTypeRepository.findByNameO(eq(UNKNOWN_EVENT_TYPE))).thenReturn(Optional.empty());
    ...
    @Test
    public void whenListPartitionsForInconsistentTopicThenInternalServerError() throws Exception {
        when(topicRepositoryMock.topicExists(eq(UNKNOWN_EVENT_TYPE))).thenReturn(false);
        when(eventTypeRepository.findByNameO(eq(UNKNOWN_EVENT_TYPE))).thenReturn(Optional.of(DUMMY_EVENT_TYPE));
    ...
    @Test
    public void whenGetPartitionForInconsistentTopicThenInternalServerError() throws Exception {
        when(topicRepositoryMock.topicExists(eq(UNKNOWN_EVENT_TYPE))).thenReturn(false);
        when(eventTypeRepository.findByNameO(eq(UNKNOWN_EVENT_TYPE))).thenReturn(Optional.of(DUMMY_EVENT_TYPE));
        final ThrowableProblem expectedProblem = Problem.valueOf(INTERNAL_SERVER_ERROR, "topic is absent in kafka");
