...
public class EventBodyMustRespectSchema extends ValidationStrategy {
  ...
  private static final Function<OverrideDefinition, QualifiedJSONSchemaValidator>
      toQualifiedJSONSchemaValidator =
          t -> {
  ...
  @Override
  public EventValidator materialize(
      final EventType eventType, final ValidationStrategyConfiguration vsc) {

    final JSONSchemaValidator defaultSchemaValidator =
        new JSONSchemaValidator(eventType.getEventTypeSchema().getSchema());
    ...
    final Function<OverrideDefinition, OverrideDefinition> enhanceWithQualifiedSchema =
        definition -> {
    ...
    final List<QualifiedJSONSchemaValidator> qualifiedValidators =
        mapper
            .convertValue(vsc.getAdditionalConfiguration(), SchemaValidationOverrides.class)
            .getOverrides()
            .stream()
...
class QualifiedJSONSchemaValidationChain implements EventValidator {
  ...
  public QualifiedJSONSchemaValidationChain(
      final List<QualifiedJSONSchemaValidator> qualifiedValidators,
      final JSONSchemaValidator defaultSchemaValidator) {
  ...
  @Override
  public boolean accepts(final JSONObject event) {
    final Predicate<QualifiedJSONSchemaValidator> matchingQualifier =
        it -> it.getQualifier().getMatch().equals(event.getString(it.getQualifier().getField()));

    final EventValidator validator =
        qualifiedValidators
            .stream()
            .filter(matchingQualifier)
            .map(EventValidator.class::cast)
            .findFirst()
            .orElse(defaultSchemaValidator);
    return validator.accepts(event);
  }
}
