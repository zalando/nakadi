...
/**
 * Extension of {@link GZIPOutputStream} to workaround for a couple of long standing JDK bugs (<a
 * href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4255743">Bug 4255743</a> and <a
 * href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4813885">Bug 4813885</a>) so the GZIP'd output can be
 * flushed.
 */
public class FlushableGZIPOutputStream extends GZIPOutputStream {
    ...
    /**
     * It is used to reserve one byte of real data so that it can be used when flushing the stream.
     */
    private byte[] lastByte = new byte[1];
    private boolean hasLastByte = false;

    /**
     * Flag that compression has to be re-enabled before the next write operation.
     */
