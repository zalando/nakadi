...
public class TestHelper {
  ...
  public boolean pushEventToPartition(
      final String topic, final String partition, final Object event) {
    String url = format("{0}/topics/{1}/partitions/{2}/events", baseUrl, topic, partition);
    final ResponseEntity<Object> responseEntity =
        restTemplate.postForEntity(url, event, Object.class);
    return responseEntity != null && responseEntity.getStatusCode() == HttpStatus.CREATED;
  }

  public void pushEventsToPartition(
      final String topic, final String partition, final Object event, final int eventNum) {
    for (int i = 0; i < eventNum; i++) {
      pushEventToPartition(topic, partition, event);
    }
  }

  public Optional<String> getOffsetForPartition(
      final List<Map<String, String>> allOffsets, final String partition) {
    return allOffsets
        .stream()
