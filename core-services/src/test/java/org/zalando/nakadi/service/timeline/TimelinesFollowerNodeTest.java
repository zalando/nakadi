...
public class TimelinesFollowerNodeTest {
    ...
    public void before() throws JsonProcessingException, InterruptedException {
        ...
        startVersion = 100L;
        zkStartVersion = 123;
        when(timelinesZookeeper.getCurrentVersion(watcherArgumentCaptor.capture()))
        ...
        node = new TimelinesFollowerNode(timelinesZookeeper, localLockManager);
        node.initializeNode();
    ...
    public void testUpdatesArePropagated() throws InterruptedException {
        ...
        TestUtils.waitFor(() -> {
                    try {
                        verify(localLockManager, times(1)).setLockedEventTypes(eq(Collections.singleton("test")));
                    } catch (InterruptedException e) {
