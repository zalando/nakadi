...
public class PartitionsControllerAT extends BaseAT {
    ...
    private void writeMessageToPartition(final int partition) throws InterruptedException, ExecutionException {
        ...
        final byte[] messageKey = "blahKey".getBytes(StandardCharsets.UTF_8);
        final byte[] message = "blahValue".getBytes(StandardCharsets.UTF_8);
        final ProducerRecord<byte[], byte[]> producerRecord = new ProducerRecord<>(
                topicName, partition, messageKey,message);
